...
-- Author: Zijun Zhao (ziz2003@med.cornell.edu)


**Announcements**   
CMS has not been set up yet for students at the NYC campus.     
Everyone should have access to Piazza. (Put a * next to your name is you do not have access)   
...
```{r, eval = FALSE}
git clone https://github.com/zijun-zhao/BTRY6830.git
```
...
```{r, comment=NA}
lab.1 <- read.table("./lab1_data.csv", sep = ",", header = TRUE)
lab.2 <- read.csv("./lab1_data.csv", header = TRUE)
```
...
```{r, comment=NA}
dim(lab.1)
head(lab.1)     # displays the first 10 rows of the data
tail(lab.1)
rownames(lab.1)
colnames(lab.1) # checking the column names of the data
```
...
```{r, comment=NA}
lab.1.only.a <- subset(lab.1, factor1 == "a")
lab.1.only.a
```
...
```{r, comment=NA}
write.table(lab.1.only.a, file = "./subset_only_a.csv", sep = ",", quote= FALSE, row.names=FALSE)
