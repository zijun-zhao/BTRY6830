...
To test the significance of genotypes in this framework, we are going to calculate the likelihood for the null and the alternative and use a likelihood ratio test similar to what we did in the linear regression lab. In order to do this, we first have to create a function to calculate the likelihood for the model fit. 
...
xa_matrix <- as.matrix(x-1)
xd_matrix <- 1 - 2*abs(xa_matrix)
covar.x <- prcomp(t(xa_matrix))$rotation[,1]
covar.y <- prcomp(t(y))$rotation[,1]
xc <- cbind(covar.x, covar.y)

for(i in 1:ncol(y)){
  beta<-NULL
  logl<-NULL
  for(j in 1:dim(xa_matrix)[2]){
  	myList<-logistic.IRLS(X=cbind(xa_matrix[,j],xd_matrix[,j]), Covars=xc, Y=as.matrix(y[,i]))
  	beta<-cbind(beta,myList[[1]])
  	logl<-c(logl,myList[[2]])
  }
  ...
  # log likelihood for NULL hypothesis
  logl_H0 <- logistic.IRLS(Y=as.matrix(y[,i]), X=rep(0, nrow(y)), Covars=xc)[[2]]
  ...
  plot(-log10(pval), main = paste("Phenotype", i))
}
